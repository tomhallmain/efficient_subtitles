...
sub_infer_and_create() { # Run sub analysis and create combined sub file with result: sub_infer_and_create video_file sub_file [overwrite_audio_samples=t] [start_minutes=6] [n_samples_per_sec=10] [intro_skip_seconds=0]
    ...
    while [[ "$optimal_samplevar" -gt 1500 && "$retry_count" -lt 9 ]]; do
        ...
        echo -e "Executing command: subs_analyze \"$1\" \"$2\" f $retry_at_minute $5 $6 | read -r optimal_samplevar_seconds_offset"
        subs_analyze "$1" "$2" f "$retry_at_minute" $5 $6 | read -r optimal_samplevar_seconds_offset
        let local optimal_samplevar="$(echo "$optimal_samplevar_seconds_offset" | awk '{print $1}')"
    ...
    if [ "$optimal_seconds_offset" ]; then
        echo -e "Executing command: get_subs_force_retimed \"$1\" \"$2\" -\"$optimal_seconds_offset\""
        get_subs_force_retimed "$1" "$2" -"$optimal_seconds_offset"
    else
        echo -e "Executing command: get_sub_with_adjustment \"$1\" \"$2\""
        get_sub_with_adjustment "$1" "$2"
    fi
}

get_sub_with_adjustment() { # Get video file with subs timing adjustment: get_sub_with_offset video_file subs_file [offset_secs_from_base] [test_duration]
    ...
    if [ ! "$1" ]
    then
        echo -e "ffmpeg -ss 00:00:06.0 -i \"${video_input}\" -vf \"ass=${sub_file}\" -b:v 20M -c:a copy ${output_file}"
        ffmpeg -ss 00:00:06.0 -i "${video_input}" -vf "ass=${sub_file}" ${_test_duration[@]} -b:v 20M -c:a copy "$output_file"
    elif $(echo "$1" | awk '{if($0 > 0) exit; else exit 1}')
    then
        local positive_offset="$1"
        ...
        local seconds_back="$(echo "$1" | sed 's#-##g')"
        echo -e "Creating new sub file at .subs_file_with_adjustment_1.ass"
        echo -e "ffmpeg -ss \"00:00:${seconds_back}\" -i \"${sub_file}\" -y .subs_file_with_adjustment.ass"
        ffmpeg -ss "00:00:${seconds_back}" -i "${sub_file}" ${_test_duration[@]} -y .subs_file_with_adjustment.ass
...
get_subs_force_retimed() { # Create new subtitle file with timing adjustment: get_subs_force_retimed video_file sub_file [secs_back] [test_duration]
    [[ -f "$1" && -f "$2" ]] || ds:fail 'Video / subtitle files not provided or invalid'
    local OLD_PWD="$(pwd)"
    local video_file_dir="$(dirname "$1")"
    cd "$video_file_dir"
    ...
    get_sub_with_adjustment "$1" "${new_sub_file}" "" "$4"
    cd "$OLD_PWD"
}

subs_analyze() { # Calculate most likely offset for subs compared to video: subs_analyze video_file sub_file [overwrite_audio_samples=t] [start_minutes=6] [n_samples_per_sec=10] [intro_skip_seconds=0]
    [[ -f "$1" && -f "$2" ]] || ds:fail 'Video / subtitle files not provided or invalid'
    local video_file="${1}"; shift
    local sub_file="${1}"
    local overwrite_audio_samples="${2:-t}"
    local start_minutes="${3:-6}"
    local n_samples_per_sec="${4:-10}"
    local intro_skip_seconds="${5:-0}"
    ...
    local GET_TIME_STRING='{
        ...
        function GetTimeString(sample_index) {
            hours = 0
            minutes = 0
            seconds = sample_index / '$n_samples_per_sec'
            if (seconds > 3600) {
      ...
      head -n 30 /tmp/subs_analyze/diff_totals | awk "$GET_TIME_STRING" >&2
      local optimal_offset_data="$(head -n1 /tmp/subs_analyze/diff_totals | awk "$GET_TIME_STRING")"
